...
func New(cfg RecoveryConfig) {
	if cfg.ErrorReporting != nil {
		...
		var err error
		errorClient, err = errorreporting.NewClient(ctx, cfg.ErrorReporting.ProjectId, errorreporting.Config{
			ServiceName: cfg.ErrorReporting.ServiceName,
		})
		if err != nil {
			log.Fatal(err)
		}
